...
HTML;
	...
	function showAuthMod($name, $value, $node, $control_name)
	{
		...
		if (!is_dir(Path::clean($plugindir . '/jfusion')) || empty($conf['jfusion'])) {
			$error = 1;
			$reason = Text::_('MOD_NOT_ENABLED');
		}
	...
	function authMod($action)
	{
		...
		switch($action) {
			...
			case 'disable':
				...
				if (is_dir(Path::clean($plugindir)) && !Folder::delete($plugindir)) {
					$error = 1;
				}
				...
				if (is_dir(Path::clean($config_path))) {
					$config_file = $config_path . 'local.php';
					if (is_file(Path::clean($config_file))) {
			...
			case 'enable':
				...
				if (Folder::copy($pluginsource, $plugindir, '', true)) {
					//update the config file
