...
class Mysql extends Basic {
		...
		if($this->dbcon && is_array($grps)) {
			...
			if($uid) {
				...
				if($gid !== false){
					 ...
					 */
					$this->_delUser($user);
					$this->debug("MySQL err: Adding user '$user' to group '$group' failed.", -1, __LINE__, __FILE__);
				}
			}
		}
		return false;
	...
	protected function _openDB() {
		if(!$this->dbcon) {
			$con = @mysql_connect($this->getConf('server'), $this->getConf('user'), $this->getConf('password'), true);
			if($con) {
				if((mysql_select_db($this->getConf('database'), $con))) {
					...
					mysql_close($con);
					$this->debug("MySQL err: No access to database {$this->getConf('database')}.", -1, __LINE__, __FILE__);
				}
			} else {
				$this->debug(
	...
	protected function _queryDB($query) {
		if($this->getConf('debug') >= 2) {
			$this->debug('MySQL query: ' . hsc($query), 0, __LINE__, __FILE__);
		}
	...
	function getConf($setting, $notset=false){
		...
		if (empty($this->conf)){ $this->loadConfig(); }
